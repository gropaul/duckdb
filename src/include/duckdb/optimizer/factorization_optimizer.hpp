...
public:
	...
	struct OperatorFactEmitInfo {
		...
		OperatorFactEmitInfo(bool is_able_p, bool is_emitting_fact_vectors_p, vector<LogicalType> flat_types_p,
		                     ...
		                     unique_ptr<ColumnBindingReplacer> replacer_p)
		    : is_able(is_able_p), is_emitting_new_fact_vectors(is_emitting_fact_vectors_p),
		      ...
		      conditions(std::move(conditions_p)), replacer(std::move(replacer_p)) {
		}
...
private:
	...
	unique_ptr<OperatorFactEmitInfo> CanEmitFactVectors(const unique_ptr<duckdb::LogicalOperator> &op,
	                                        const vector<unique_ptr<OperatorFactEmitInfo>> &children_info);

	unique_ptr<OperatorFactEmitInfo> EmitFactVectors(unique_ptr<LogicalOperator> &op,
	...
	void AddFactExpandBeforeOperatorAndChild(unique_ptr<duckdb::LogicalOperator> &parent,
	                                         ...
	                                         vector<FactExpandCondition> &conditions);
	bool OperatorCanProcessFactVectors(const LogicalOperator &op, idx_t child_idx, unique_ptr<OperatorFactEmitInfo> &child_info);
};

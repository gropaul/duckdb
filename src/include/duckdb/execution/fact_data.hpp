...
inline static idx_t CalculateHTCapacity(idx_t chain_length) {
	return MaxValue<idx_t>(NextPowerOfTwo(chain_length * 2), 16);
}

struct fact_data_t { // NOLINT
	void Initialize(idx_t chain_length_p, idx_t ht_capacity_p, data_ptr_t chain_head_p, data_ptr_t *pointers_p, uint64_t *keys_p) {
	...
	inline idx_t GetHTCapacity() const {
		return ht_capacity;
	}

	inline idx_t GetHTBitmask() const {
		return ht_bitmask;
	}
};

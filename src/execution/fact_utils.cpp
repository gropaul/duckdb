...
static void LoadKeysIfNecessary(UnifiedVectorFormat &offsets_uvf, JoinHashTable *ht, column_t key_column,
                                const SelectionVector &sel, idx_t count, JoinHashTable::FactProbeState &probe_state) {
	...
	for (idx_t idx = 0; idx < count; idx++) {
		...
		if (PARALLEL) {
			...
			bool needs_loading = atomic_fact_data_states[fact_data_offset].compare_exchange_weak(
			...
			bool is_loading = atomic_fact_data_states[fact_data_offset].load(std::memory_order_relaxed) ==
			                  FactDataState::KEYS_LOADING_IN_PROGRESS;
			probe_state.chains_to_wait_for_sel.set_index(remaining_chains_to_wait_for, sel_idx);
			remaining_chains_to_wait_for += is_loading;
		} else {
			// if we are not in parallel mode, we can just load the keys if they are not already loaded
			bool needs_loading = atomic_fact_data_states[fact_data_offset] == FactDataState::KEYS_NOT_LOADED;
			probe_state.chains_to_load_sel.set_index(remaining_chains_to_load, sel_idx);
			remaining_chains_to_load += needs_loading;
		}
	}
		...
		for (idx_t idx = 0; idx < remaining_chains_to_load; idx++) {
			...
			if (chain_element_index < fact_data.chain_length) {
				...
				if (PARALLEL) {
					...
					atomic_fact_data_states[fact_offset] = FactDataState::KEYS_LOADED;
				}
			}
		}
	...
	if (PARALLEL) {
		...
		for (idx_t idx = 0; idx < remaining_chains_to_wait_for; idx++) {
			...
			while (atomic_fact_data_states[fact_data_offset].load(std::memory_order_relaxed) ==
			       FactDataState::KEYS_LOADING_IN_PROGRESS) {
			}
		}
	}
...
static void BuildFactHTIfNecessary(fact_data_t &fact_data, atomic<FactDataState> &state) {
	...
	if (PARALLEL){
		...
		bool needs_building = state.compare_exchange_weak(expected, desired, std::memory_order_relaxed);
		...
		if (state == FactDataState::KEYS_LOADED) {
			state = FactDataState::HT_BUILT_COMPLETED;
			fact_data.BuildHT();
		}
	}
...
static bool DetermineSidesAndBuildHT(const JoinHashTable *lhs_ht, const JoinHashTable *rhs_ht,
	...
	BuildFactHTIfNecessary<PARALLEL>(rhs_fact_data, rhs_state);
	return false; // Build on the rhs side -> no swap needed
}

static void IntersectFacts(const fact_data_t &lhs_fact_data, const fact_data_t &rhs_fact_data,
                           data_ptr_t *lhs_pointers_res, data_ptr_t *rhs_pointers_res, idx_t &intersection_count) {
...
template <bool PARALLEL>
static void BuildAndIntersectFacts(const JoinHashTable *lhs_ht, const JoinHashTable *rhs_ht,
                                   const idx_t lhs_fact_data_offset, const idx_t rhs_fact_data_offset,
                                   data_ptr_t *lhs_pointers_res, data_ptr_t *rhs_pointers_res,
	...
	bool swap_sides = DetermineSidesAndBuildHT<PARALLEL>(lhs_ht, rhs_ht, lhs_fact_data_offset, rhs_fact_data_offset,
